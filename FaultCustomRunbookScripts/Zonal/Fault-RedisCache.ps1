...
.PARAMETER ResourceIds
    Comma-separated Redis Cache resource IDs. Each resource id should be in the format:
...
[CmdletBinding()]
param (
    [Parameter(Mandatory = $true, HelpMessage = "Comma-separated Redis Cache resource IDs.")]
...
$functions = {
    ...
    function Initialize-RequiredModules {
        ...
        try {
            ...
            if (-not (Get-Module -Name "Az.RedisCache" -ListAvailable 4>$null)) {
            ...
            if (-not (Get-Module -Name "Az.RedisCache")) {
                ...
                Import-Module Az.RedisCache -Force -ErrorAction Stop 4>$null
            }
...
$cacheTargets = Get-ResourceTargets -ResourceIds $ResourceIds -SubscriptionToTargetZone $SubscriptionToTargetZone -TargetZone $TargetZone
Write-Verbose "Parsed $($cacheTargets.Count) Redis cache target(s)."
...
Write-Verbose "Starting parallel processing of $($cacheTargets.Count) Redis caches"

$functionsScript = $functions.ToString()

$resultsRaw = $cacheTargets | ForEach-Object -Parallel {
    ...
    $VerbosePreference = 'SilentlyContinue'
    Import-Module Az.Accounts -ErrorAction Stop
    Import-Module Az.RedisCache -ErrorAction Stop
    $VerbosePreference = 'Continue'
...
if ($failureCount -gt 0) {
    $errorMsg = "Runbook failed: $failureCount out of $($cacheTargets.Count) Redis cache(s) could not be faulted. Status: $overallStatus"
